...
$(document).ready(function() {
    // Namespace para validación de creación
    const CreateValidator = {
        ...
        checkNombreCursoDisponible: async function(nombre) {
            try {
                const response = await $.ajax({
                    url: '/Sistema%20de%20Control%20de%20Participantes/app/controlador/Curso/verificarCurso.php',
                    method: 'GET',
        ...
        validateNombreCurso: async function() {
            ...
            if (nombre === '') {
                this.showError('nombre_curso', 'El nombre del curso es requerido');
                return false;
            }
            if (nombre.match(/\d/)) {
                this.showError('nombre_curso', 'No se permiten números');
                return false;
            }
            if (nombre.startsWith(' ') || nombre.endsWith(' ')) {
                this.showError('nombre_curso', 'No se permiten espacios al inicio/final');
                return false;
            }
            if (!regex.test(nombre)) {
                this.showError('nombre_curso', 'Solo letras y caracteres especiales válidos');
                return false;
            }
            
            const existe = await this.checkNombreCursoDisponible(nombre);
            if (existe) {
                this.showError('nombre_curso', 'Este curso ya está registrado');
                return false;
            }
            
            this.showSuccess('nombre_curso');
            return true;
        },
        ...
        validateFechas: function() {
            ...
            if (!inicio) {
                this.showError('fecha_inicio', 'Seleccione fecha de inicio');
                isValid = false;
            } else if (inicio < hoy) {
                this.showError('fecha_inicio', 'No puede ser fecha pasada');
                isValid = false;
            } else {
                this.showSuccess('fecha_inicio');
            }

            if (!fin) {
                this.showError('fecha_fin', 'Seleccione fecha de fin');
                isValid = false;
            } else {
                ...
                if (fechaFin < fechaInicio) {
                    this.showError('fecha_fin', 'Debe ser posterior al inicio');
                    isValid = false;
                } else if (diferencia < semanasMinimas) {
                    this.showError('fecha_fin', 'Debe haber al menos 3 semanas entre las fechas');
                    isValid = false;
                } else {
                    this.showSuccess('fecha_fin');
                }
            }
            return isValid;
        },
        ...
        validateParticipantes: function() {
            const participantes = $('#max_participantes').val();
            if (!participantes || participantes < 10 || participantes > 40) {
                this.showError('max_participantes', 'Debe ser entre 10 y 40');
                return false;
            }
            this.showSuccess('max_participantes');
            return true;
        }
    };

    // Eventos de validación para creación
    $('#nombre_curso').on('input', async function() {
        $(this).val($(this).val().replace(/[0-9]/g, '').trim());
        await CreateValidator.validateNombreCurso();
    });
    
    $('#id_docente, #id_modalidad, #id_motor, #id_areaFormativa, #sectoreconomico, #subtipo, #ambito, #turno')
  .on('change', function() {
    const fieldId = $(this).attr('id');
    CreateValidator.validateSelect(fieldId, 'Campo requerido');
});

    
    $('#fecha_inicio, #fecha_fin').on('change', () => CreateValidator.validateFechas());
    
    $('#max_participantes').on('input', function() {
        $(this).val($(this).val().replace(/\D/g, ''));
        CreateValidator.validateParticipantes();
    });

    // Manejo del envío del formulario de creación
    $('#form-curso').on('submit', async function(e) {
        e.preventDefault();
        
        const validations = [
            await CreateValidator.validateNombreCurso(),
            CreateValidator.validateSelect('id_docente', 'Seleccione un docente'),  
            CreateValidator.validateSelect('id_modalidad', 'Seleccione modalidad'),
            CreateValidator.validateSelect('id_motor', 'Seleccione motor'),
            CreateValidator.validateSelect('id_areaFormativa', 'Seleccione área formativa'),
            CreateValidator.validateSelect('sectoreconomico', 'Seleccione sector económico'),
            CreateValidator.validateSelect('subtipo', 'Seleccione subtipo'),
            CreateValidator.validateSelect('ambito', 'Seleccione ámbito'),
            CreateValidator.validateSelect('turno', 'Seleccione turno'),
            CreateValidator.validateFechas(),
            CreateValidator.validateParticipantes()
        ];

        if (validations.every(v => v === true)) {
            const formData = $(this).serializeArray();
            formData.push({ name: 'id_estatusCurso', value: 1 });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#modal-addCurso').modal('hide');
                        $('#form-curso')[0].reset();
                        $('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
                        $('.error-feedback, .valid-feedback').hide();
                        toastr.success("El curso se ha registrado correctamente");
                        cargarTablaCursos();
                    } else {
                        if (response.errors) {
                            Object.entries(response.errors).forEach(([field, message]) => {
                                CreateValidator.showError(field, message);
                            });
                        }
                        toastr.error(response.message || 'Error al registrar');
                    }
                },
                error: function(xhr) {
                    toastr.error('Error en el servidor: ' + xhr.responseText);
                }
            });
        } else {
    ...
    $('#modal-addCurso').on('hidden.bs.modal', function() {
        $('#form-curso')[0].reset();
        $('.is-invalid, .is-valid').removeClass('is-invalid is-valid');
        $('.error-feedback, .valid-feedback').hide();
    });
